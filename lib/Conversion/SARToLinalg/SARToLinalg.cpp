...
struct ConstOpConverterPattern final : public OpConversionPattern<mlir::sar::ConstOp> {
	...
	LogicalResult matchAndRewrite(mlir::sar::ConstOp op, OpAdaptor adaptor,
		...
		Attribute rawAttr = op.getValueAttr();
		TypedAttr typedAttr = dyn_cast<TypedAttr>(rawAttr);
		if (!typedAttr) {
			if (auto s = dyn_cast<StringAttr>(rawAttr)) {
				Attribute parsed = mlir::parseAttribute(s.getValue(), rewriter.getContext());
				typedAttr = dyn_cast_if_present<TypedAttr>(parsed);
			}
		}
		if (!typedAttr)
			return rewriter.notifyMatchFailure(op, "const value is not a TypedAttr");
		rewriter.replaceOpWithNewOp<arith::ConstantOp>(op.getOperation(), newType, typedAttr);
		return success();
	}
};
	...
	LogicalResult matchAndRewrite(SarOp op, typename SarOp::Adaptor adaptor,
		...
		auto genericOp = rewriter.create<linalg::GenericOp>(
			...
			[&](OpBuilder &b, Location loc, ValueRange args) {
				...
				} else if (llvm::isa<mlir::ComplexType>(elemType)) {
					if (std::is_same<SarOp, sar::ElemAddOp>::value) {
						result = b.create<complex::AddOp>(loc, args[0], args[1]);
					} else if (std::is_same<SarOp, sar::ElemSubOp>::value) {
						result = b.create<complex::SubOp>(loc, args[0], args[1]);
					} else if (std::is_same<SarOp, sar::ElemMulOp>::value) {
						result = b.create<complex::MulOp>(loc, args[0], args[1]);
					} else if (std::is_same<SarOp, sar::ElemDivOp>::value) {
						result = b.create<complex::DivOp>(loc, args[0], args[1]);
					}
				}
